...
2)   Create New Order
3)   Update Order Status - Incomplete
...
def update_order_status():
    ...
    df = pandas.read_csv('orders.csv', index_col=0)
    print(df)
    inputted_id = input("Please input the Order ID:")
    for info in orders:
        if(info["Order_ID"] == inputted_id):
            info["Status"] = "Delivered"
        elif (info["Order_ID"] != inputted_id):
            print("You entered an incorrect Order Number...")
            order_return_option()
